        ...
        while True:
            try:
                ...
                if connection_heap:
                    ...
                    logger.info("Connection heap empty, reloading pending connections")
                    load_pending_connections()
                    time.sleep(30)
...
def add_to_heap(connection):
    ...
    if conn_id not in connection_in_heap:
        ...
        heapq.heappush(connection_heap, (connection.end_date, counter, conn_id))
        connection_in_heap.add(conn_id)
        logger.info(f"Added connection {conn_id} to heap, scheduled for {connection.end_date}")
